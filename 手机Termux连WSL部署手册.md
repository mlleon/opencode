...
**B. 长内容（跨屏）⭐**：滑动 → `Space` → 滚动 → `Enter`

1. **手指滑动**屏幕 → 自动进入 Copy Mode（右上角出现 `[x/总行]` 行号）
2. 滑动到起始位置
3. **按 `Space`** 锁定起点（光标处，精准）
4. **手指上下滑动**翻页 → 选择延伸（被选文字变黄色），可跨屏
5. 滑到终点后**按 `Enter`** → 复制并退出 Copy Mode
6. 到任意 app 粘贴

> 跨屏方案靠 `Space` 锁起点 + 滚动移动光标延伸选择，避开触屏拖拽到边缘失效的硬伤。`Space`/`Enter` 是手机软键盘主键，直接可按。
