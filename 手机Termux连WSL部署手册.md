...
tmux 默认开鼠标（手指滑动翻历史），但鼠标开着会拦截触屏选文字。复制流程：

1. 按 `Ctrl+b m` 关鼠标（底部显示 `mouse: OFF`）
2. **长按**终端 → 出现选择锚点
3. **拖动锚点**选范围（真文字选择，精准）
4. **复制** → 进手机剪贴板
5. 到任意 app（微信/备忘录/其他终端）粘贴
6. 按 `Ctrl+b m` 开回鼠标，恢复滑动翻历史

> 详细说明见 [tmux-手册.md 第九章](tmux-手册.md)。
...
- 滚动看历史：手指上下滑（鼠标开）
- 复制文本：`Ctrl+b m` 关鼠标 → 长按选文字 → 复制 → `Ctrl+b m` 开回来

> 向日葵方案遗留的 `F8`/`v`/`y`/`clip.exe` 配置已全部删除——SSH 方案下复制靠 Termux 原生长按，进手机剪贴板，不需要那些。
...
复制：`Ctrl+b m` 关鼠标 → 长按选文字 → 复制 → `Ctrl+b m` 开回来。
