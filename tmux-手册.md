...
2. **滑动到起始位置**（把光标移到要复制的起点）
3. **按 `Space`** → 在光标处锁定选择起点
4. **手指上下滑动**翻页 → 光标移动，选择跟着延伸（**被选文字变黄色高亮**），可跨屏
5. 滑到终点后**按 `Enter`** → 复制并退出 Copy Mode
6. 到任意 app 长按 → 粘贴
...
**为什么 Space/Enter 在手机上好按**：`Space` 和 `Enter` 是手机软键盘主键区的标准键，直接可按，不需要切换符号页（这是相比 `v`/`y`/`[` 等键的优势）。
...
```tmux
# vi 模式：启用 Space（开始选择）+ Enter（复制）默认绑定
setw -g mode-keys vi
```
...
| 起点不准                   | 光标位置没对齐                 | 滑动时看右上角行号定位，或放大字体 |
| 想重新选                   | 直接滑动 + Space 重新开始      | 不用退出 Copy Mode                |
