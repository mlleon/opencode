...
| 功能            | 键                        |
| --------------- | ------------------------- |
| 进入 Copy Mode  | `Ctrl+b [`（或手指滑动自动进入）              |
| 搜索            | `/`                     |
| 下一个 / 上一个 | `n` / `N`             |
| 开始选择        | `v` 或 `Space`（手机用 Space）     |
| 复制并退出      | `y` 或 `Enter`（手机用 Enter）     |
| 翻页            | `PageUp` / `PageDown` |
| 跳顶 / 跳底     | `g` / `G`             |
| 退出            | `q`                     |

> ⚠️ **电脑端复制不用 Copy Mode**，直接鼠标拖选即可（写 Windows 剪贴板）。手机端短内容用「长按选文字」，跨屏用「Space + 滚动 + Enter」。详见第九章。
...
> - **手机端**（Termux SSH）：两种复制方式
>   - 短内容（当前屏）：`Ctrl+b m` + 长按选文字
...
| 场景              | 方案                                   | 复制范围     |
| ----------------- | -------------------------------------- | ------------ |
| 短内容（当前屏）  | `Ctrl+b m` 关鼠标 → 长按选文字         | 仅当前屏     |
| 长内容（跨屏） ⭐ | 滑动进 Copy Mode → `Space` → 滚动 → `Enter` | 任意长度 |

两种方式都依赖 `~/.tmux.conf` 的配置（见第十章）：`mouse on` + `prefix m` 切换 + `mode-keys vi`。

### 9.3 滚动看历史（鼠标开）

手指在 Termux 里上下滑 = 翻 tmux scrollback。和电脑端鼠标滚轮等效。

> 手指滑动时 tmux 会**自动进入 Copy Mode**（右上角出现 `[当前行/总行]` 行号），这是滚轮绑定的默认行为，不用手动按 `Ctrl+b [`。

### 9.4 当前屏复制：Ctrl+b m + 长按（短内容）
...
> **局限**：只能选当前屏幕可见内容，不能跨屏。跨屏用 9.5。

### 9.5 跨屏复制：Space + 滚动 + Enter（长内容）⭐

适合复制大段日志、跨屏的 Agent 输出。**核心思路**：用 `Space` 在光标处锁定起点（键盘精度），用滚动移动光标延伸选择（可跨屏），避开触屏拖拽到边缘失效的硬伤。

**完整流程**：

1. **手指滑动**屏幕 → 自动进入 Copy Mode（右上角出现 `[x/总行]` 行号）
2. **滑动到起始位置附近**（让目标起始行出现在屏幕上可见即可，不用精确）
3. **用方向键微调** → 把光标精确移到起始行（Termux 自带方向键，比 `j`/`k` 更顺手）
4. **按 `Space`** → 在光标处锁定选择起点
5. **手指上下滑动**翻页 → 光标移动，选择跟着延伸（**被选文字变黄色高亮**），可跨屏
6. 滑到终点附近，**再用方向键微调**到精确结束行
...
滑动 → 进 Copy Mode
滑动到起点附近        ← 只求目标行出现在屏幕上
方向键微调             ← 把光标精确对到起始行（解决滑动光标钉屏不动的根因）
Space                 ← 锁起点（现在精准了）
滑动跨屏              ← 选择延伸（黄色高亮跟着光标走）
方向键微调             ← 对到结束行
Enter                 ← 复制 + 退出
...
tmux 3.4 的 Copy Mode 里，**滚动时光标钉在屏幕固定位置不动，是内容在光标下流动**（实测验证）。所以「滑动到期望位置」时，**眼睛看的行 ≠ 光标所在的行**，直接按 Space 起点会偏。必须滑动到大致位置后，用方向键把光标移到目标行（此时光标和目标行在同一屏内，按几下就对齐了）。

**为什么能跨屏**（而触屏拖拽不能）：

| 操作         | 机制                           | 能否跨屏             |
| ------------ | ------------------------------ | -------------------- |
| 触屏拖拽     | 拖选择终点，到边缘 Android 截手势 | ❌ 只能当前屏        |
| Space + 滚动 | 滚动移动光标，选择跟着光标延伸 | ✅ 整个 scrollback   |

**为什么 Space/Enter/方向键在手机上好按**：`Space` 和 `Enter` 是手机软键盘主键区的标准键；方向键 Termux 自带（额外键行），都不需要切换符号页（这是相比 `v`/`y`/`[` 等键的优势）。

**复制到哪个剪贴板**：`Enter` 触发 `copy-pipe-and-cancel`（vi 默认绑定），配合 `set-clipboard external`（OSC 52）转发到 Termux → **手机自己的剪贴板**。

**配置**（`~/.tmux.conf`）：

```tmux
# vi 模式：启用 Space（开始选择）+ Enter（复制）+ 方向键（移动光标）默认绑定
setw -g mode-keys vi
```
...
| 现象                       | 原因                          | 处理                              |
| -------------------------- | ----------------------------- | --------------------------------- |
| 按 Space 没反应            | mode-keys 不是 vi（可能 WSL 重启后配置丢失） | 确认 `~/.tmux.conf` 有 `setw -g mode-keys vi`，`tmux source-file ~/.tmux.conf` |
| 选到一半选择消失           | 不小心滑到最底部自动退出 Copy Mode | 重新滑动进 Copy Mode，重来        |
| 起点不准                   | 滑动后光标钉屏不动，没对齐     | 滑动到大致位置后用**方向键微调**光标到目标行，再按 Space |
| 想重新选                   | 直接滑动 + 方向键 + Space 重新开始 | 不用退出 Copy Mode                |

### 9.6 关键事实：电脑端 vs 手机端复制目标不同

| 端     | 复制方式                    | 复制到哪个剪贴板       | 说明                               |
| ------ | --------------------------- | ---------------------- | ---------------------------------- |
| 电脑端 | 鼠标拖选                    | Windows 系统剪贴板     | 通过 `clip.exe` 写入               |
| 手机端 | `Ctrl+b m` + 长按，或 Space + Enter | **手机自己的剪贴板** | 通过 Termux 原生选择 / OSC 52 写入 |

和向日葵方案的根本差异（向日葵已弃用）：向日葵复制进 Windows 剪贴板，SSH 方案下手机端复制进手机剪贴板。

### 9.7 Copy Mode 什么时候还用得到

**电脑端**：偶尔用 `Ctrl+b [` 进 Copy Mode，键盘翻历史 + 搜索（`/`、`n`）。日常复制用鼠标拖选（见 9.0）。

**手机端**：跨屏复制就是用 Copy Mode（9.5 的 Space + 滚动 + Enter 流程本质是在 vi Copy Mode 里操作）。`Ctrl+b [` 手动进入已非必需——手指滑动会自动进入。
...
# 手机端复制（当前屏）：prefix+m 关鼠标 → 长按选文字 → 复制 → prefix+m 开回来
# 手机端复制（跨屏）：滑动进 Copy Mode → Space → 滚动 → Enter（见 mode-keys vi）
set -g mouse on
...
# ─── 复制优化 ─────────────────────────────────────────────
# 电脑端鼠标拖选松手：写 Windows 剪贴板，留在 copy-mode（不自动退出，可继续滚）
bind-key -T copy-mode MouseDragEnd1Pane send-keys -X copy-pipe "clip.exe"

# 手机端跨屏复制：vi Copy Mode（Space 开始选择 / Enter 复制，均为 vi 默认绑定）
# 配合 set-clipboard external（OSC 52）转发到 Termux 手机剪贴板
setw -g mode-keys vi
```
...
> **关于配置演进**：早期向日葵方案的 `v`/`y`/`F8` 复合绑定已删除。`mode-keys vi` 重新启用（手机端跨屏复制需要 Space/Enter 默认绑定，见 9.5）。`clip.exe` 在电脑端复制场景下引入（鼠标拖选写 Windows 剪贴板）。备份在 `~/.tmux.conf.bak.20260806`。
